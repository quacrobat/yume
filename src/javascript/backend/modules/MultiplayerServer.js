/**
 * @file Creates and manages a WebSocket-Server for handling
...
/**
 * Event listener for the "connection" event of the WebSockets API. When a user
 * connects to the server, this method assigns the user to a session and adds
 * event listeners to the corresponding socket. The socket object represents the
...
/**
 * This method assigns the socket-object to a session. A session is something
 * like a chat-room. Messages of a client will only delivered to other clients
 * in the same session. This method always tries to assign users to existing
